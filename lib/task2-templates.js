...
/**
 * Merge sentence rows that share the exact same `text`, regardless of which
 * selection mode produced `rows` (by type, by paragraph, by function, by
 * dimension, ...). Wording is identical across several essay types for many
 * slots (e.g. the neutral "background" sentence used by types 1-6 and 9), so
 * without this step the same sentence would be displayed once per essay
 * type that reuses it.
 *
 * The first occurrence's position and metadata (slot, section, blanks,
 * note_zh, ...) become the representative row; every essay type that shares
 * the wording is collected into `typeIds` so the UI can show them all (e.g.
 * "1,2,3,4,5,6,9") instead of a single type number.
 * @param {Task2Row[]} rows
 * @returns {(Task2Row & { typeIds: number[] })[]}
 */
export function dedupeRowsByText(rows) {
  const groupByText = new Map();
  const orderedGroups = [];

  rows.forEach((row) => {
    const existingGroup = groupByText.get(row.text);
    if (existingGroup) {
      if (!existingGroup.typeIds.includes(row.type_id)) {
        existingGroup.typeIds.push(row.type_id);
      }
      return;
    }
    const newGroup = { ...row, typeIds: [row.type_id] };
    groupByText.set(row.text, newGroup);
